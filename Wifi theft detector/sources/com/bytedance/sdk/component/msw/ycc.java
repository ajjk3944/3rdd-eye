...
public class ycc extends bw {
    ...
    public static ExecutorService ypw() {
        if (bw == null) {
            synchronized (ycc.class) {
                try {
                    if (bw == null) {
                        bw = new emc.C0119emc().emc("init").emc(0).ypw(10).emc(5L).emc(TimeUnit.SECONDS).emc(new SynchronousQueue()).emc(ycc()).emc(bw.emc().emc(10, "init")).emc();
                    }
                } catch (Throwable th) {
                    throw th;
                }
    ...
    public static ExecutorService emc(int i10) {
        if (ycc == null) {
            synchronized (ycc.class) {
                try {
                    if (ycc == null) {
                        emc emcVarEmc = new emc.C0119emc().emc("io").emc(2).ypw(i10).emc(20L).emc(TimeUnit.SECONDS).emc(new LinkedBlockingQueue()).emc(ycc()).emc(bw.emc().emc(i10, "io")).emc();
                        ycc = emcVarEmc;
    ...
    public static void emc(msw mswVar, int i10) {
        ypw(mswVar);
    }
