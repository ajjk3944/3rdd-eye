...
public class ycc {
    ...
    public void emc(Context context, boolean z10, rie rieVar, String str, Map<String, String> map, com.bytedance.sdk.openadsdk.core.widget.bw bwVar) {
        if (map == null || map.isEmpty()) {
            return;
        }
        try {
            if (emc(Integer.parseInt(map.get("landingStyle")), map.get("url"), map.get("fallbackUrl"))) {
                sba.emc(context, z10, emc(map), rieVar, str, tp.emc(str), null, bwVar);
            }
        } catch (Throwable th) {
