...
public abstract class AbstractC1704ew {
     ...
     */
    public static WebResourceResponse A00(C1814gi c1814gi, WebResourceRequest webResourceRequest, Uri uri, String str, HashMap<String, String> map) throws JSONException, IOException {
        String strA02 = A02(85, 5, 1);
        C5Y c5yA0H = C1745fb.A06(c1814gi.A02()).A0H(c1814gi);
        C1220Sx.A0H(c1814gi, uri.toString());
        try {
            C1705ex c1705ex = new C1705ex(c1814gi.A02(), uri, c5yA0H);
            int iAvailable = c1705ex.available();
            if (iAvailable <= 0) {
                A05(c1814gi, 1, new Pair[]{new Pair(A02(61, 9, 46), String.valueOf(iAvailable))});
                return null;
            }
            ...
            if (strA03 == null) {
                c1814gi.A0F().A6D();
                A06(map, iAvailable);
                return new WebResourceResponse(str, null, 200, A02(44, 2, 122), map, c1705ex);
            }
            try {
                C1703ev c1703evA01 = A01(strA03);
                if (!c1703evA01.A03) {
                    A05(c1814gi, 0, new Pair[]{new Pair(A02(94, 5, 77), c1703evA01.A02 != null ? c1703evA01.A02 : A02(90, 4, 30))});
                    return null;
                }
                int i10 = c1703evA01.A01;
                int i11 = c1703evA01.A00 == -1 ? iAvailable - 1 : c1703evA01.A00;
                A06(map, iAvailable);
                map.put(A02(31, 13, 93), A02(75, 6, 3) + i10 + A02(1, 1, 92) + i11 + A02(2, 1, 88) + iAvailable);
                c1814gi.A0F().A6D();
                return new WebResourceResponse(str, null, Sdk$SDKError.Reason.AD_ALREADY_FAILED_VALUE, A02(46, 15, 10), map, c1705ex);
            } catch (NumberFormatException e10) {
                A05(c1814gi, 3, new Pair[]{new Pair(strA02, e10.toString())});
                return null;
            }
        } catch (IOException e11) {
            A05(c1814gi, 2, new Pair[]{new Pair(strA02, e11.toString())});
            return null;
        }
    }
    ...
    java.lang.ArrayIndexOutOfBoundsException
     */
    public static void A05(C1814gi c1814gi, int i10, Pair<String, String>[] pairArr) throws JSONException {
        ...
        c1814gi.A0F().A6C(jSONObject.toString());
    }
    ...
    public static C1703ev A01(String str) {
        if (str == null) {
            C1703ev parseResult = new C1703ev();
        ...
        if (strArrSplit.length >= 2) {
            if (A02(70, 5, 5).equals(strArrSplit[0].toLowerCase(Locale.US).trim())) {
                if (strArrSplit[1].trim().split(A02(0, 1, 72)).length != 1) {
                    C1703ev c1703ev = new C1703ev();
                    c1703ev.A03 = false;
                    c1703ev.A02 = str;
                    return c1703ev;
                }
                String[] ranges = strArrSplit[1].trim().split(A02(1, 1, 92));
                C1703ev c1703ev2 = new C1703ev();
                c1703ev2.A03 = true;
                c1703ev2.A02 = str;
                c1703ev2.A01 = TextUtils.isEmpty(ranges[0]) ? 0 : Integer.parseInt(ranges[0]);
                if (ranges.length > 1) {
                    c1703ev2.A00 = TextUtils.isEmpty(ranges[1]) ? -1 : Integer.parseInt(ranges[1]);
                } else {
                    c1703ev2.A00 = -1;
                }
                return c1703ev2;
