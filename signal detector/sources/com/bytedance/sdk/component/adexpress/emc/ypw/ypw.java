...
public class ypw {
    ...
    private static boolean bw(String str) {
        ...
        List<emc.C0024emc> listYcc;
        if (!bw() || (emcVarDg = dg()) == null || (listYcc = emcVarDg.ycc()) == null) {
            return false;
        }
        for (emc.C0024emc c0024emc : listYcc) {
            if (c0024emc != null && TextUtils.equals(str, c0024emc.emc())) {
                return true;
            }
        }
        return false;
    ...
    private static File ycc(String str) throws IOException {
        if (bw()) {
            Iterator<emc.C0024emc> it = dg().ycc().iterator();
            while (true) {
                if (!it.hasNext()) {
                    break;
                }
                emc.C0024emc next = it.next();
