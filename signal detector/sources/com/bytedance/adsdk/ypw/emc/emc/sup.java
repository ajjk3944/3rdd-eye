...
public class sup implements ru, sz {
    ...
    public Path dg() {
        ...
        int i = AnonymousClass1.emc[this.ycc.ypw().ordinal()];
        if (i == 1) {
            emc();
        } else if (i == 2) {
            emc(Path.Op.UNION);
    ...
    public void emc(List<xq> list, List<xq> list2) {
        for (int i = 0; i < this.bw.size(); i++) {
            this.bw.get(i).emc(list, list2);
        }
    }

    private void emc() {
        for (int i = 0; i < this.bw.size(); i++) {
            this.xq.addPath(this.bw.get(i).dg());
        }
    }
    ...
    private void emc(Path.Op op) {
        ...
        if (szVar2 instanceof dg) {
            ...
            for (int i = 0; i < listYpw2.size(); i++) {
                Path pathDg2 = listYpw2.get(i).dg();
