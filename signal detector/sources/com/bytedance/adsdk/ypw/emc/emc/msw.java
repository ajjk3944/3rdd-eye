...
public class msw implements bw, gbl, emc.InterfaceC0021emc {
    private com.bytedance.adsdk.ypw.emc.ypw.aa aa;
    ...
    private RadialGradient xq() {
        ...
        float f2 = pointFUym.x;
        float f5 = pointFUym.y;
        float fHypot = (float) Math.hypot(pointFUym2.x - f2, pointFUym2.y - f5);
        if (fHypot <= 0.0f) {
            fHypot = 0.001f;
        }
        RadialGradient radialGradient2 = new RadialGradient(f2, f5, fHypot, iArrEmc, fArrEmc, Shader.TileMode.CLAMP);
    ...
    public void emc(Canvas canvas, Matrix matrix, int i) {
        ...
        for (int i3 = 0; i3 < this.ru.size(); i3++) {
            this.uym.addPath(this.ru.get(i3).dg(), matrix);
        }
        ...
        this.msw.setAlpha(com.bytedance.adsdk.ypw.ycc.bw.emc((int) ((((i / 255.0f) * this.sz.uym().intValue()) / 100.0f) * 255.0f), 0, 255));
    ...
    public void emc(RectF rectF, Matrix matrix, boolean z6) {
        ...
        for (int i = 0; i < this.ru.size(); i++) {
            this.uym.addPath(this.ru.get(i).dg(), matrix);
        }
