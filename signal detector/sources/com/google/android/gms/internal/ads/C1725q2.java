package com.google.android.gms.internal.ads;

import android.util.Pair;
import android.util.SparseArray;
import androidx.recyclerview.widget.C0305b;
import java.math.RoundingMode;
import java.util.ArrayDeque;
import java.util.Arrays;
import java.util.Collections;
import java.util.List;
import java.util.PriorityQueue;

/* renamed from: com.google.android.gms.internal.ads.q2, reason: case insensitive filesystem */
/* loaded from: classes.dex */
public final class C1725q2 implements InterfaceC1992v0 {

    /* renamed from: L, reason: collision with root package name */
    public static final byte[] f16274L = {-94, 57, 79, 82, 90, -101, 79, 20, -94, 68, 108, 66, 124, 100, -115, -12};
    public static final TP M;

    /* renamed from: A, reason: collision with root package name */
    public C1671p2 f16275A;

    /* renamed from: B, reason: collision with root package name */
    public int f16276B;

    /* renamed from: C, reason: collision with root package name */
    public int f16277C;

    /* renamed from: D, reason: collision with root package name */
    public int f16278D;

    /* renamed from: E, reason: collision with root package name */
    public boolean f16279E;

    /* renamed from: F, reason: collision with root package name */
    public boolean f16280F;

    /* renamed from: G, reason: collision with root package name */
    public InterfaceC2100x0 f16281G;

    /* renamed from: H, reason: collision with root package name */
    public T0[] f16282H;

    /* renamed from: I, reason: collision with root package name */
    public T0[] f16283I;

    /* renamed from: J, reason: collision with root package name */
    public boolean f16284J;

    /* renamed from: K, reason: collision with root package name */
    public long f16285K;

    /* renamed from: a, reason: collision with root package name */
    public final R2 f16286a;

    /* renamed from: b, reason: collision with root package name */
    public final int f16287b;

    /* renamed from: c, reason: collision with root package name */
    public final List f16288c;

    /* renamed from: h, reason: collision with root package name */
    public final byte[] f16293h;
    public final C2036vr i;

    /* renamed from: n, reason: collision with root package name */
    public final C0305b f16298n;

    /* renamed from: o, reason: collision with root package name */
    public final C1723q0 f16299o;

    /* renamed from: p, reason: collision with root package name */
    public C1197gC f16300p;

    /* renamed from: q, reason: collision with root package name */
    public int f16301q;

    /* renamed from: r, reason: collision with root package name */
    public int f16302r;

    /* renamed from: s, reason: collision with root package name */
    public long f16303s;

    /* renamed from: t, reason: collision with root package name */
    public int f16304t;

    /* renamed from: u, reason: collision with root package name */
    public C2036vr f16305u;

    /* renamed from: v, reason: collision with root package name */
    public long f16306v;

    /* renamed from: w, reason: collision with root package name */
    public int f16307w;

    /* renamed from: x, reason: collision with root package name */
    public long f16308x;

    /* renamed from: y, reason: collision with root package name */
    public long f16309y;

    /* renamed from: z, reason: collision with root package name */
    public long f16310z;

    /* renamed from: j, reason: collision with root package name */
    public final C0889ae f16294j = new C0889ae(3);

    /* renamed from: k, reason: collision with root package name */
    public final C2036vr f16295k = new C2036vr(16);

    /* renamed from: e, reason: collision with root package name */
    public final C2036vr f16290e = new C2036vr(AbstractC2022vd.f17310z);

    /* renamed from: f, reason: collision with root package name */
    public final C2036vr f16291f = new C2036vr(6);

    /* renamed from: g, reason: collision with root package name */
    public final C2036vr f16292g = new C2036vr();

    /* renamed from: l, reason: collision with root package name */
    public final ArrayDeque f16296l = new ArrayDeque();

    /* renamed from: m, reason: collision with root package name */
    public final ArrayDeque f16297m = new ArrayDeque();

    /* renamed from: d, reason: collision with root package name */
    public final SparseArray f16289d = new SparseArray();

    static {
        C1855sP c1855sP = new C1855sP();
        c1855sP.e("application/x-emsg");
        M = new TP(c1855sP);
    }

    public C1725q2(R2 r22, int i, C1197gC c1197gC) {
        this.f16286a = r22;
        this.f16287b = i;
        this.f16288c = Collections.unmodifiableList(c1197gC);
        byte[] bArr = new byte[16];
        this.f16293h = bArr;
        this.i = new C2036vr(bArr);
        JB jb = LB.f9635b;
        this.f16300p = C1197gC.f14227e;
        this.f16309y = -9223372036854775807L;
        this.f16308x = -9223372036854775807L;
        this.f16310z = -9223372036854775807L;
        this.f16281G = InterfaceC2100x0.f17531A;
        this.f16282H = new T0[0];
        this.f16283I = new T0[0];
        this.f16298n = new C0305b(new L6(3, this));
        this.f16299o = new C1723q0();
        this.f16285K = -1L;
    }

    public static void i(int i) throws P4 {
        if (i >= 0) {
            return;
        }
        StringBuilder sb = new StringBuilder(String.valueOf(i).length() + 27);
        sb.append("Unexpected negative value: ");
        sb.append(i);
        throw P4.a(null, sb.toString());
    }

    public static void j(C2036vr c2036vr, int i, B2 b22) throws P4 {
        c2036vr.E(i + 8);
        int iB = c2036vr.b();
        byte[] bArr = AbstractC1455l2.f15333a;
        if ((iB & 1) != 0) {
            throw P4.b("Overriding TrackEncryptionBox parameters is unsupported.");
        }
        boolean z6 = (iB & 2) != 0;
        int iH = c2036vr.h();
        if (iH == 0) {
            Arrays.fill(b22.f7112l, 0, b22.f7106e, false);
            return;
        }
        int i3 = b22.f7106e;
        C2036vr c2036vr2 = b22.f7114n;
        if (iH != i3) {
            throw P4.a(null, A.f.o(new StringBuilder(String.valueOf(iH).length() + 58 + String.valueOf(i3).length()), "Senc sample count ", iH, " is different from fragment sample count", i3));
        }
        Arrays.fill(b22.f7112l, 0, iH, z6);
        c2036vr2.y(c2036vr.B());
        b22.f7111k = true;
        b22.f7115o = true;
        c2036vr.H(0, c2036vr2.f17356c, c2036vr2.f17354a);
        c2036vr2.E(0);
        b22.f7115o = false;
    }

    public static Pair k(long j6, C2036vr c2036vr) throws P4 {
        long j7;
        long j8;
        C2036vr c2036vr2 = c2036vr;
        c2036vr2.E(8);
        int iA = AbstractC1455l2.a(c2036vr2.b());
        c2036vr2.G(4);
        long jP = c2036vr2.P();
        if (iA == 0) {
            j7 = c2036vr2.P();
            j8 = c2036vr2.P();
        } else {
            j7 = c2036vr2.j();
            j8 = c2036vr2.j();
        }
        long j9 = j8 + j6;
        long jU = Vt.u(j7, 1000000L, jP, RoundingMode.DOWN);
        c2036vr2.G(2);
        int iL = c2036vr2.L();
        int[] iArr = new int[iL];
        long[] jArr = new long[iL];
        long[] jArr2 = new long[iL];
        long[] jArr3 = new long[iL];
        long j10 = j9;
        long j11 = jU;
        int i = 0;
        while (i < iL) {
            int iB = c2036vr2.b();
            if ((Integer.MIN_VALUE & iB) != 0) {
                throw P4.a(null, "Unhandled indirect reference");
            }
            long jP2 = c2036vr2.P();
            iArr[i] = iB & Integer.MAX_VALUE;
            jArr[i] = j10;
            jArr3[i] = j11;
            j7 += jP2;
            long[] jArr4 = jArr2;
            long[] jArr5 = jArr3;
            long jU2 = Vt.u(j7, 1000000L, jP, RoundingMode.DOWN);
            jArr4[i] = jU2 - jArr5[i];
            c2036vr2.G(4);
            j10 += iArr[i];
            i++;
            iL = iL;
            c2036vr2 = c2036vr;
            j11 = jU2;
            jArr2 = jArr4;
            jArr3 = jArr5;
        }
        return Pair.create(Long.valueOf(jU), new C1669p0(iArr, jArr, jArr2, jArr3));
    }

    /* JADX WARN: Removed duplicated region for block: B:30:0x0114  */
    /* JADX WARN: Removed duplicated region for block: B:31:0x0117  */
    /* JADX WARN: Removed duplicated region for block: B:33:0x011d  */
    /* JADX WARN: Removed duplicated region for block: B:34:0x0125  */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public static com.google.android.gms.internal.ads.SO l(java.util.List r18) {
        /*
            Method dump skipped, instructions count: 333
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.C1725q2.l(java.util.List):com.google.android.gms.internal.ads.SO");
    }

    public final void a() {
        this.f16301q = 0;
        this.f16304t = 0;
    }

    @Override // com.google.android.gms.internal.ads.InterfaceC1992v0
    public final boolean b(InterfaceC2046w0 interfaceC2046w0) {
        C1197gC c1197gCI;
        Q0 q0P = AbstractC1114el.p(interfaceC2046w0, true, false);
        if (q0P != null) {
            c1197gCI = LB.i(q0P);
        } else {
            JB jb = LB.f9635b;
            c1197gCI = C1197gC.f14227e;
        }
        this.f16300p = c1197gCI;
        return q0P == null;
    }

    @Override // com.google.android.gms.internal.ads.InterfaceC1992v0
    public final /* synthetic */ List c() {
        return this.f16300p;
    }

    @Override // com.google.android.gms.internal.ads.InterfaceC1992v0
    public final void d() {
    }

    @Override // com.google.android.gms.internal.ads.InterfaceC1992v0
    public final void e(InterfaceC2100x0 interfaceC2100x0) {
        if ((this.f16287b & 32) == 0) {
            interfaceC2100x0 = new A1.s(interfaceC2100x0, this.f16286a);
        }
        this.f16281G = interfaceC2100x0;
        a();
        T0[] t0Arr = new T0[2];
        this.f16282H = t0Arr;
        int i = 0;
        T0[] t0Arr2 = (T0[]) Vt.m(0, t0Arr);
        this.f16282H = t0Arr2;
        for (T0 t02 : t0Arr2) {
            t02.e(M);
        }
        List list = this.f16288c;
        this.f16283I = new T0[list.size()];
        int i3 = 100;
        while (i < this.f16283I.length) {
            int i6 = i3 + 1;
            T0 t0X = this.f16281G.x(i3, 3);
            t0X.e((TP) list.get(i));
            this.f16283I[i] = t0X;
            i++;
            i3 = i6;
        }
    }

    @Override // com.google.android.gms.internal.ads.InterfaceC1992v0
    public final void f(long j6, long j7) {
        SparseArray sparseArray = this.f16289d;
        int size = sparseArray.size();
        for (int i = 0; i < size; i++) {
            ((C1671p2) sparseArray.valueAt(i)).a();
        }
        this.f16297m.clear();
        this.f16307w = 0;
        ((PriorityQueue) this.f16298n.f5434e).clear();
        this.f16308x = j7;
        this.f16296l.clear();
        a();
    }

    /* JADX WARN: Code restructure failed: missing block: B:100:0x01e6, code lost:
    
        if ((r4 + r2) <= (r35.f16276B - r35.f16277C)) goto L102;
     */
    /* JADX WARN: Code restructure failed: missing block: B:102:0x01e9, code lost:
    
        r2 = r2;
     */
    /* JADX WARN: Code restructure failed: missing block: B:103:0x01eb, code lost:
    
        r19 = r4;
        r32 = r7;
        ((com.google.android.gms.internal.ads.C1776r0) r36).w(r8, r12, r4 + r2, false);
        r6.E(0);
        r4 = r6.b();
     */
    /* JADX WARN: Code restructure failed: missing block: B:104:0x01ff, code lost:
    
        if (r4 < 0) goto L502;
     */
    /* JADX WARN: Code restructure failed: missing block: B:105:0x0201, code lost:
    
        r35.f16278D = r4 - r2;
        r4 = r35.f16290e;
        r4.E(0);
        r3.a(4, r4);
        r35.f16277C += 4;
        r35.f16276B += r12;
     */
    /* JADX WARN: Code restructure failed: missing block: B:106:0x021a, code lost:
    
        if (r35.f16283I.length <= 0) goto L120;
     */
    /* JADX WARN: Code restructure failed: missing block: B:107:0x021c, code lost:
    
        if (r2 <= 0) goto L120;
     */
    /* JADX WARN: Code restructure failed: missing block: B:108:0x021e, code lost:
    
        r4 = r8[4];
        r7 = com.google.android.gms.internal.ads.AbstractC2022vd.W(r5);
     */
    /* JADX WARN: Code restructure failed: missing block: B:109:0x0228, code lost:
    
        if (j$.util.Objects.equals(r7, "video/avc") == false) goto L113;
     */
    /* JADX WARN: Code restructure failed: missing block: B:110:0x022a, code lost:
    
        r18 = r4;
     */
    /* JADX WARN: Code restructure failed: missing block: B:111:0x022f, code lost:
    
        if ((r4 & 31) == 6) goto L118;
     */
    /* JADX WARN: Code restructure failed: missing block: B:113:0x0232, code lost:
    
        r18 = r4;
     */
    /* JADX WARN: Code restructure failed: missing block: B:115:0x0239, code lost:
    
        if (j$.util.Objects.equals(r7, "video/hevc") == false) goto L119;
     */
    /* JADX WARN: Code restructure failed: missing block: B:117:0x0243, code lost:
    
        if (((r18 & 126) >> 1) != 39) goto L119;
     */
    /* JADX WARN: Code restructure failed: missing block: B:118:0x0245, code lost:
    
        r7 = true;
     */
    /* JADX WARN: Code restructure failed: missing block: B:119:0x0247, code lost:
    
        r7 = false;
     */
    /* JADX WARN: Code restructure failed: missing block: B:121:0x024b, code lost:
    
        r35.f16280F = r7;
        r3.a(r2, r6);
        r35.f16277C += r2;
     */
    /* JADX WARN: Code restructure failed: missing block: B:122:0x0255, code lost:
    
        if (r2 <= 0) goto L128;
     */
    /* JADX WARN: Code restructure failed: missing block: B:124:0x0259, code lost:
    
        if (r35.f16279E != false) goto L128;
     */
    /* JADX WARN: Code restructure failed: missing block: B:126:0x025f, code lost:
    
        if (com.google.android.gms.internal.ads.AbstractC2022vd.C(r8, r2, r5) == false) goto L128;
     */
    /* JADX WARN: Code restructure failed: missing block: B:127:0x0261, code lost:
    
        r35.f16279E = true;
     */
    /* JADX WARN: Code restructure failed: missing block: B:128:0x0264, code lost:
    
        r2 = r17;
        r4 = r19;
     */
    /* JADX WARN: Code restructure failed: missing block: B:129:0x0268, code lost:
    
        r7 = r32;
     */
    /* JADX WARN: Code restructure failed: missing block: B:131:0x0273, code lost:
    
        throw com.google.android.gms.internal.ads.P4.a(null, "Invalid NAL length");
     */
    /* JADX WARN: Code restructure failed: missing block: B:132:0x0274, code lost:
    
        r19 = r4;
        r32 = r7;
     */
    /* JADX WARN: Code restructure failed: missing block: B:133:0x027b, code lost:
    
        if (r35.f16280F == false) goto L145;
     */
    /* JADX WARN: Code restructure failed: missing block: B:134:0x027d, code lost:
    
        r7 = r35.f16292g;
        r7.y(r2);
        r24 = r6;
        ((com.google.android.gms.internal.ads.C1776r0) r36).w(r7.f17354a, 0, r35.f16278D, false);
        r3.a(r35.f16278D, r7);
        r2 = r35.f16278D;
        r4 = com.google.android.gms.internal.ads.AbstractC2022vd.b(r7.f17356c, r7.f17354a);
        r7.E(0);
        r7.C(r4);
        r4 = r5.f11399o;
     */
    /* JADX WARN: Code restructure failed: missing block: B:135:0x02a7, code lost:
    
        if (r4 != (-1)) goto L139;
     */
    /* JADX WARN: Code restructure failed: missing block: B:137:0x02ab, code lost:
    
        if (r11.f5430a == 0) goto L142;
     */
    /* JADX WARN: Code restructure failed: missing block: B:138:0x02ad, code lost:
    
        r11.v(0);
     */
    /* JADX WARN: Code restructure failed: missing block: B:140:0x02b3, code lost:
    
        if (r11.f5430a == r4) goto L142;
     */
    /* JADX WARN: Code restructure failed: missing block: B:141:0x02b5, code lost:
    
        r11.v(r4);
     */
    /* JADX WARN: Code restructure failed: missing block: B:142:0x02b8, code lost:
    
        r11.w(r13, r7);
     */
    /* JADX WARN: Code restructure failed: missing block: B:143:0x02c4, code lost:
    
        if ((r17.b() & 4) == 0) goto L146;
     */
    /* JADX WARN: Code restructure failed: missing block: B:144:0x02c6, code lost:
    
        r11.x(0);
     */
    /* JADX WARN: Code restructure failed: missing block: B:145:0x02ca, code lost:
    
        r24 = r6;
        r2 = r3.f(r36, r2, false);
     */
    /* JADX WARN: Code restructure failed: missing block: B:146:0x02d3, code lost:
    
        r35.f16277C += r2;
        r35.f16278D -= r2;
        r2 = r17;
        r4 = r19;
        r6 = r24;
     */
    /* JADX WARN: Code restructure failed: missing block: B:147:0x02e4, code lost:
    
        r0 = r17.b();
     */
    /* JADX WARN: Code restructure failed: missing block: B:148:0x02ea, code lost:
    
        if (r35.f16279E != false) goto L150;
     */
    /* JADX WARN: Code restructure failed: missing block: B:149:0x02ec, code lost:
    
        r0 = r0 | 67108864;
     */
    /* JADX WARN: Code restructure failed: missing block: B:150:0x02ef, code lost:
    
        r27 = r0;
        r0 = r17.e();
     */
    /* JADX WARN: Code restructure failed: missing block: B:151:0x02f5, code lost:
    
        if (r0 == null) goto L153;
     */
    /* JADX WARN: Code restructure failed: missing block: B:152:0x02f7, code lost:
    
        r30 = r0.f6919c;
     */
    /* JADX WARN: Code restructure failed: missing block: B:153:0x02fc, code lost:
    
        r30 = null;
     */
    /* JADX WARN: Code restructure failed: missing block: B:154:0x02fe, code lost:
    
        r25 = r13;
        r3.c(r25, r27, r35.f16276B, 0, r30);
     */
    /* JADX WARN: Code restructure failed: missing block: B:156:0x030f, code lost:
    
        if (r32.isEmpty() != false) goto L497;
     */
    /* JADX WARN: Code restructure failed: missing block: B:157:0x0311, code lost:
    
        r0 = (com.google.android.gms.internal.ads.C1617o2) r32.removeFirst();
        r2 = r35.f16307w;
        r7 = r0.f15887c;
        r35.f16307w = r2 - r7;
        r2 = r0.f15885a;
     */
    /* JADX WARN: Code restructure failed: missing block: B:158:0x0322, code lost:
    
        if (r0.f15886b == false) goto L160;
     */
    /* JADX WARN: Code restructure failed: missing block: B:159:0x0324, code lost:
    
        r2 = r2 + r25;
     */
    /* JADX WARN: Code restructure failed: missing block: B:160:0x0326, code lost:
    
        r4 = r2;
        r0 = r35.f16282H;
        r2 = r0.length;
        r10 = 0;
     */
    /* JADX WARN: Code restructure failed: missing block: B:161:0x032b, code lost:
    
        if (r10 >= r2) goto L500;
     */
    /* JADX WARN: Code restructure failed: missing block: B:162:0x032d, code lost:
    
        r0[r10].c(r4, 1, r7, r35.f16307w, null);
        r10 = r10 + 1;
     */
    /* JADX WARN: Code restructure failed: missing block: B:164:0x033d, code lost:
    
        if (r17.c() != false) goto L68;
     */
    /* JADX WARN: Code restructure failed: missing block: B:165:0x033f, code lost:
    
        r35.f16275A = null;
     */
    /* JADX WARN: Code restructure failed: missing block: B:166:0x0344, code lost:
    
        r35.f16301q = 3;
     */
    /* JADX WARN: Code restructure failed: missing block: B:167:0x0348, code lost:
    
        return 0;
     */
    /* JADX WARN: Code restructure failed: missing block: B:275:0x06c6, code lost:
    
        throw com.google.android.gms.internal.ads.P4.b("Atom size less than header length (unsupported).");
     */
    /* JADX WARN: Code restructure failed: missing block: B:43:0x00ba, code lost:
    
        r3 = r2.f16098a;
        r4 = r2.f16099b;
     */
    /* JADX WARN: Code restructure failed: missing block: B:44:0x00c7, code lost:
    
        if (r35.f16301q != 3) goto L77;
     */
    /* JADX WARN: Code restructure failed: missing block: B:46:0x00cb, code lost:
    
        if (r2.f16109m != false) goto L48;
     */
    /* JADX WARN: Code restructure failed: missing block: B:47:0x00cd, code lost:
    
        r6 = r2.f16101d.f7496d[r2.f16103f];
     */
    /* JADX WARN: Code restructure failed: missing block: B:48:0x00d6, code lost:
    
        r6 = r4.f7109h[r2.f16103f];
     */
    /* JADX WARN: Code restructure failed: missing block: B:49:0x00dc, code lost:
    
        r35.f16276B = r6;
        r6 = r2.f16101d.f7493a.f17814g.f11397m;
     */
    /* JADX WARN: Code restructure failed: missing block: B:50:0x00ea, code lost:
    
        if (j$.util.Objects.equals(r6, "video/avc") != false) goto L52;
     */
    /* JADX WARN: Code restructure failed: missing block: B:51:0x00ec, code lost:
    
        j$.util.Objects.equals(r6, "video/hevc");
     */
    /* JADX WARN: Code restructure failed: missing block: B:52:0x00ef, code lost:
    
        r35.f16279E = true;
     */
    /* JADX WARN: Code restructure failed: missing block: B:53:0x00f6, code lost:
    
        if (r2.f16103f >= r2.i) goto L69;
     */
    /* JADX WARN: Code restructure failed: missing block: B:54:0x00f8, code lost:
    
        ((com.google.android.gms.internal.ads.C1776r0) r36).b(r35.f16276B, false);
        r0 = r2.e();
     */
    /* JADX WARN: Code restructure failed: missing block: B:55:0x0103, code lost:
    
        if (r0 != null) goto L57;
     */
    /* JADX WARN: Code restructure failed: missing block: B:57:0x0106, code lost:
    
        r3 = r4.f7114n;
        r0 = r0.f6920d;
     */
    /* JADX WARN: Code restructure failed: missing block: B:58:0x010a, code lost:
    
        if (r0 == 0) goto L60;
     */
    /* JADX WARN: Code restructure failed: missing block: B:59:0x010c, code lost:
    
        r3.G(r0);
     */
    /* JADX WARN: Code restructure failed: missing block: B:60:0x010f, code lost:
    
        r0 = r2.f16103f;
     */
    /* JADX WARN: Code restructure failed: missing block: B:61:0x0113, code lost:
    
        if (r4.f7111k == false) goto L65;
     */
    /* JADX WARN: Code restructure failed: missing block: B:63:0x0119, code lost:
    
        if (r4.f7112l[r0] == false) goto L65;
     */
    /* JADX WARN: Code restructure failed: missing block: B:64:0x011b, code lost:
    
        r3.G(r3.L() * 6);
     */
    /* JADX WARN: Code restructure failed: missing block: B:66:0x0127, code lost:
    
        if (r2.c() != false) goto L68;
     */
    /* JADX WARN: Code restructure failed: missing block: B:67:0x0129, code lost:
    
        r35.f16275A = null;
     */
    /* JADX WARN: Code restructure failed: missing block: B:70:0x0136, code lost:
    
        if (r2.f16101d.f7493a.f17815h != 1) goto L72;
     */
    /* JADX WARN: Code restructure failed: missing block: B:71:0x0138, code lost:
    
        r35.f16276B -= 8;
        ((com.google.android.gms.internal.ads.C1776r0) r36).b(8, false);
     */
    /* JADX WARN: Code restructure failed: missing block: B:73:0x0154, code lost:
    
        if ("audio/ac4".equals(r2.f16101d.f7493a.f17814g.f11397m) == false) goto L75;
     */
    /* JADX WARN: Code restructure failed: missing block: B:74:0x0156, code lost:
    
        r35.f16277C = r2.d(r35.f16276B, 7);
        com.google.android.gms.internal.ads.AbstractC1114el.r(r35.f16276B, r5);
        r3.a(7, r5);
        r5 = r35.f16277C + 7;
        r35.f16277C = r5;
     */
    /* JADX WARN: Code restructure failed: missing block: B:75:0x016d, code lost:
    
        r5 = r2.d(r35.f16276B, 0);
        r35.f16277C = r5;
     */
    /* JADX WARN: Code restructure failed: missing block: B:76:0x0175, code lost:
    
        r35.f16276B += r5;
        r35.f16301q = 4;
        r35.f16278D = 0;
     */
    /* JADX WARN: Code restructure failed: missing block: B:77:0x017e, code lost:
    
        r5 = r2.f16101d;
        r6 = r5.f7493a;
     */
    /* JADX WARN: Code restructure failed: missing block: B:78:0x0184, code lost:
    
        if (r2.f16109m != false) goto L80;
     */
    /* JADX WARN: Code restructure failed: missing block: B:79:0x0186, code lost:
    
        r13 = r5.f7498f[r2.f16103f];
     */
    /* JADX WARN: Code restructure failed: missing block: B:80:0x018d, code lost:
    
        r13 = r4.i[r2.f16103f];
     */
    /* JADX WARN: Code restructure failed: missing block: B:81:0x0193, code lost:
    
        r4 = r6.f17817k;
        r5 = r6.f17814g;
     */
    /* JADX WARN: Code restructure failed: missing block: B:82:0x0197, code lost:
    
        if (r4 != 0) goto L88;
     */
    /* JADX WARN: Code restructure failed: missing block: B:83:0x0199, code lost:
    
        r4 = r35.f16277C;
        r5 = r35.f16276B;
     */
    /* JADX WARN: Code restructure failed: missing block: B:84:0x019d, code lost:
    
        if (r4 >= r5) goto L496;
     */
    /* JADX WARN: Code restructure failed: missing block: B:85:0x019f, code lost:
    
        r35.f16277C += r3.f(r36, r5 - r4, false);
     */
    /* JADX WARN: Code restructure failed: missing block: B:86:0x01aa, code lost:
    
        r17 = r2;
     */
    /* JADX WARN: Code restructure failed: missing block: B:87:0x01ac, code lost:
    
        r32 = r7;
     */
    /* JADX WARN: Code restructure failed: missing block: B:88:0x01b0, code lost:
    
        r6 = r35.f16291f;
        r8 = r6.f17354a;
        r8[0] = 0;
        r8[1] = 0;
        r8[r18] = 0;
        r12 = 4 - r4;
     */
    /* JADX WARN: Code restructure failed: missing block: B:89:0x01be, code lost:
    
        r17 = r2;
     */
    /* JADX WARN: Code restructure failed: missing block: B:90:0x01c4, code lost:
    
        if (r35.f16277C >= r35.f16276B) goto L501;
     */
    /* JADX WARN: Code restructure failed: missing block: B:91:0x01c6, code lost:
    
        r2 = r35.f16278D;
     */
    /* JADX WARN: Code restructure failed: missing block: B:92:0x01c8, code lost:
    
        if (r2 != 0) goto L132;
     */
    /* JADX WARN: Code restructure failed: missing block: B:94:0x01cd, code lost:
    
        if (r35.f16283I.length > 0) goto L99;
     */
    /* JADX WARN: Code restructure failed: missing block: B:96:0x01d1, code lost:
    
        if (r35.f16279E != false) goto L98;
     */
    /* JADX WARN: Code restructure failed: missing block: B:98:0x01d4, code lost:
    
        r2 = 0;
     */
    /* JADX WARN: Code restructure failed: missing block: B:99:0x01d6, code lost:
    
        r2 = com.google.android.gms.internal.ads.AbstractC2022vd.s(r5);
     */
    @Override // com.google.android.gms.internal.ads.InterfaceC1992v0
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public final int g(com.google.android.gms.internal.ads.InterfaceC2046w0 r36, com.google.android.gms.internal.ads.A0 r37) throws java.lang.NumberFormatException, java.io.IOException {
        /*
            Method dump skipped, instructions count: 2241
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.C1725q2.g(com.google.android.gms.internal.ads.w0, com.google.android.gms.internal.ads.A0):int");
    }

    /* JADX WARN: Code restructure failed: missing block: B:308:0x0794, code lost:
    
        a();
     */
    /* JADX WARN: Code restructure failed: missing block: B:309:0x0797, code lost:
    
        return;
     */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public final void h(long r54) throws com.google.android.gms.internal.ads.P4, java.lang.NumberFormatException {
        /*
            Method dump skipped, instructions count: 1944
            To view this dump change 'Code comments level' option to 'DEBUG'
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.internal.ads.C1725q2.h(long):void");
    }
}
