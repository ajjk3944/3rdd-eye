...
public abstract class AbstractC1326ew {
     ...
     */
    public static WebResourceResponse A00(C1436gi c1436gi, WebResourceRequest webResourceRequest, Uri uri, String str, HashMap<String, String> map) throws JSONException, IOException {
        String strA02 = A02(85, 5, 1);
        C5Y c5yA0H = C1367fb.A06(c1436gi.A02()).A0H(c1436gi);
        C0842Sx.A0H(c1436gi, uri.toString());
        try {
            C1327ex c1327ex = new C1327ex(c1436gi.A02(), uri, c5yA0H);
            int iAvailable = c1327ex.available();
            if (iAvailable <= 0) {
                A05(c1436gi, 1, new Pair[]{new Pair(A02(61, 9, 46), String.valueOf(iAvailable))});
                return null;
            }
            ...
            if (strA03 == null) {
                c1436gi.A0F().A6D();
                A06(map, iAvailable);
                return new WebResourceResponse(str, null, TTAdConstant.MATE_VALID, A02(44, 2, 122), map, c1327ex);
            }
            try {
                C1325ev c1325evA01 = A01(strA03);
                if (!c1325evA01.A03) {
                    A05(c1436gi, 0, new Pair[]{new Pair(A02(94, 5, 77), c1325evA01.A02 != null ? c1325evA01.A02 : A02(90, 4, 30))});
                    return null;
                }
                int i4 = c1325evA01.A01;
                int i10 = c1325evA01.A00 == -1 ? iAvailable - 1 : c1325evA01.A00;
                A06(map, iAvailable);
                map.put(A02(31, 13, 93), A02(75, 6, 3) + i4 + A02(1, 1, 92) + i10 + A02(2, 1, 88) + iAvailable);
                c1436gi.A0F().A6D();
                return new WebResourceResponse(str, null, 206, A02(46, 15, 10), map, c1327ex);
            } catch (NumberFormatException e2) {
                A05(c1436gi, 3, new Pair[]{new Pair(strA02, e2.toString())});
                return null;
            }
        } catch (IOException e10) {
            A05(c1436gi, 2, new Pair[]{new Pair(strA02, e10.toString())});
            return null;
        }
    }
    ...
    java.lang.ArrayIndexOutOfBoundsException
     */
    public static void A05(C1436gi c1436gi, int i4, Pair<String, String>[] pairArr) throws JSONException {
        JSONObject jSONObject = new JSONObject();
        try {
            jSONObject.put(A02(81, 4, 57), i4);
            for (Pair<String, String> pair : pairArr) {
                jSONObject.put((String) pair.first, pair.second);
            }
        } catch (JSONException unused) {
        }
        c1436gi.A0F().A6C(jSONObject.toString());
    }
    ...
    public static C1325ev A01(String str) {
        if (str == null) {
            C1325ev parseResult = new C1325ev();
        ...
        if (strArrSplit.length >= 2) {
            if (A02(70, 5, 5).equals(strArrSplit[0].toLowerCase(Locale.US).trim())) {
                if (strArrSplit[1].trim().split(A02(0, 1, 72)).length != 1) {
                    C1325ev c1325ev = new C1325ev();
                    c1325ev.A03 = false;
                    c1325ev.A02 = str;
                    return c1325ev;
                }
                String[] ranges = strArrSplit[1].trim().split(A02(1, 1, 92));
                C1325ev c1325ev2 = new C1325ev();
                c1325ev2.A03 = true;
                c1325ev2.A02 = str;
                c1325ev2.A01 = TextUtils.isEmpty(ranges[0]) ? 0 : Integer.parseInt(ranges[0]);
                if (ranges.length > 1) {
                    c1325ev2.A00 = TextUtils.isEmpty(ranges[1]) ? -1 : Integer.parseInt(ranges[1]);
                } else {
                    c1325ev2.A00 = -1;
                }
                return c1325ev2;
    ...
    public static void A06(HashMap<String, String> map, int i4) {
        map.put(A02(4, 13, 61), A02(70, 5, 5));
        map.put(A02(17, 14, 93), String.valueOf(i4));
    }
}
