package ch;

import com.liuzh.deviceinfo.pro.account.mode.ApiException;
import com.liuzh.deviceinfo.utilities.y;
import ek.j;
import java.security.NoSuchAlgorithmException;
import km.s;
import yj.l;
import yj.u;

/* compiled from: r8-map-id-c2d6645a5534910d782496a2f1fbb8d7e389c4692d5500a02d24b9efa19c0cfc */
/* loaded from: classes3.dex */
public final class c extends j implements mk.e {

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ int f2859b;

    /* renamed from: c, reason: collision with root package name */
    public int f2860c;

    /* renamed from: d, reason: collision with root package name */
    public /* synthetic */ Object f2861d;

    /* renamed from: e, reason: collision with root package name */
    public final /* synthetic */ String f2862e;

    /* renamed from: f, reason: collision with root package name */
    public final /* synthetic */ String f2863f;
    public final /* synthetic */ String g;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ c(String str, String str2, int i4, String str3, ck.c cVar) {
        super(2, cVar);
        this.f2859b = i4;
        this.f2862e = str;
        this.f2863f = str2;
        this.g = str3;
    }

    @Override // ek.a
    public final ck.c create(Object obj, ck.c cVar) {
        switch (this.f2859b) {
            case 0:
                String str = this.g;
                c cVar2 = new c(this.f2862e, this.f2863f, 0, str, cVar);
                cVar2.f2861d = obj;
                return cVar2;
            default:
                String str2 = this.g;
                c cVar3 = new c(this.f2862e, this.f2863f, 1, str2, cVar);
                cVar3.f2861d = obj;
                return cVar3;
        }
    }

    @Override // mk.e
    public final Object invoke(Object obj, Object obj2) {
        al.i iVar = (al.i) obj;
        ck.c cVar = (ck.c) obj2;
        switch (this.f2859b) {
        }
        return ((c) create(iVar, cVar)).invokeSuspend(u.f37649a);
    }

    @Override // ek.a
    public final Object invokeSuspend(Object obj) throws NoSuchAlgorithmException {
        Object objK;
        Object objI;
        switch (this.f2859b) {
            case 0:
                al.i iVar = (al.i) this.f2861d;
                int i4 = this.f2860c;
                u uVar = u.f37649a;
                dk.a aVar = dk.a.f22275a;
                try {
                } catch (s e2) {
                    l lVar = new l(ci.b.h(new ApiException(e2.f28456a, e2.f28457b)));
                    this.f2861d = null;
                    this.f2860c = 2;
                    if (iVar.c(lVar, this) != aVar) {
                        return uVar;
                    }
                }
                if (i4 == 0) {
                    ci.b.D(obj);
                    f fVar = e.f2868a;
                    String str = this.f2862e;
                    String str2 = this.f2863f;
                    String strH = y.h(this.g.concat("com.liuzh.deviceinfo"));
                    this.f2861d = iVar;
                    this.f2860c = 1;
                    objK = fVar.k(str, str2, strH, 1, this);
                    if (objK == aVar) {
                    }
                    return aVar;
                }
                if (i4 != 1) {
                    if (i4 != 2 && i4 != 3 && i4 != 4) {
                        throw new IllegalStateException("call to 'resume' before 'invoke' with coroutine");
                    }
                    ci.b.D(obj);
                    return uVar;
                }
                ci.b.D(obj);
                objK = obj;
                fh.e eVar = (fh.e) objK;
                if (eVar.d()) {
                    l lVar2 = new l(eVar);
                    this.f2861d = null;
                    this.f2860c = 3;
                    if (iVar.c(lVar2, this) != aVar) {
                        return uVar;
                    }
                } else {
                    l lVar3 = new l(ci.b.h(new ApiException(eVar.c(), null, 2, null)));
                    this.f2861d = null;
                    this.f2860c = 4;
                    if (iVar.c(lVar3, this) != aVar) {
                        return uVar;
                    }
                }
                return aVar;
            default:
                al.i iVar2 = (al.i) this.f2861d;
                int i10 = this.f2860c;
                u uVar2 = u.f37649a;
                dk.a aVar2 = dk.a.f22275a;
                try {
                } catch (s e10) {
                    l lVar4 = new l(ci.b.h(new ApiException(e10.f28456a, e10.f28457b)));
                    this.f2861d = null;
                    this.f2860c = 2;
                    if (iVar2.c(lVar4, this) != aVar2) {
                        return uVar2;
                    }
                }
                if (i10 == 0) {
                    ci.b.D(obj);
                    f fVar2 = e.f2868a;
                    String str3 = this.f2862e;
                    String str4 = this.f2863f;
                    String strH2 = y.h(this.g.concat("com.liuzh.deviceinfo"));
                    this.f2861d = iVar2;
                    this.f2860c = 1;
                    objI = fVar2.i(str3, str4, strH2, this);
                    if (objI == aVar2) {
                    }
                    return aVar2;
                }
                if (i10 != 1) {
                    if (i10 != 2 && i10 != 3 && i10 != 4) {
                        throw new IllegalStateException("call to 'resume' before 'invoke' with coroutine");
                    }
                    ci.b.D(obj);
                    return uVar2;
                }
                ci.b.D(obj);
                objI = obj;
                fh.e eVar2 = (fh.e) objI;
                if (eVar2.d()) {
                    l lVar5 = new l(uVar2);
                    this.f2861d = null;
                    this.f2860c = 3;
                    if (iVar2.c(lVar5, this) != aVar2) {
                        return uVar2;
                    }
                } else {
                    l lVar6 = new l(ci.b.h(new ApiException(eVar2.c(), null, 2, null)));
                    this.f2861d = null;
                    this.f2860c = 4;
                    if (iVar2.c(lVar6, this) != aVar2) {
                        return uVar2;
                    }
                }
                return aVar2;
        }
    }
}
