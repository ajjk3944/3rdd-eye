...
public abstract class OpenSSLEvpCipher extends OpenSSLCipher {
    ...
    int doFinalInternal(byte[] bArr, int i10, int i11) throws BadPaddingException, IllegalBlockSizeException, ShortBufferException {
        ...
        if (length >= i11) {
            iEVP_CipherFinal_ex = NativeCrypto.EVP_CipherFinal_ex(this.cipherCtx, bArr, i10);
        } else {
            ...
            if (iEVP_CipherFinal_ex2 > 0) {
                System.arraycopy(bArr2, 0, bArr, i10, iEVP_CipherFinal_ex2);
            }
            iEVP_CipherFinal_ex = iEVP_CipherFinal_ex2;
        }
        reset();
        return (iEVP_CipherFinal_ex + i10) - i10;
    }
    ...
    int updateInternal(byte[] bArr, int i10, int i11, byte[] bArr2, int i12, int i13) throws ShortBufferException {
        ...
        if (length >= i13) {
            int iEVP_CipherUpdate = NativeCrypto.EVP_CipherUpdate(this.cipherCtx, bArr2, i12, bArr, i10, i11) + i12;
            this.calledUpdate = true;
            return iEVP_CipherUpdate - i12;
        }
