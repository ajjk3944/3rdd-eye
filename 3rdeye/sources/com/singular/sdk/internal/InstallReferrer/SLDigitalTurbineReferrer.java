...
public class SLDigitalTurbineReferrer implements SLInstallReferrerService {
    ...
    private String readContentProvider(Context context, String str, ContentProviderClient contentProviderClient) {
        try {
            ...
            Cursor cursorQuery = contentProviderClient != null ? contentProviderClient.query(uri, strArr, "package_name=?", new String[]{context.getPackageName()}, null) : context.getContentResolver().query(uri, strArr, null, null, null);
            if (cursorQuery == null) {
    ...
    private String readContentProviderIntentAction(Context context, String str) {
        try {
            List<ResolveInfo> listQueryIntentContentProviders = Build.VERSION.SDK_INT >= 33 ? context.getPackageManager().queryIntentContentProviders(new Intent(Constants.DT_CONTENT_PROVIDER_INTENT_ACTION_NAME), PackageManager.ResolveInfoFlags.of(131072L)) : context.getPackageManager().queryIntentContentProviders(new Intent(Constants.DT_CONTENT_PROVIDER_INTENT_ACTION_NAME), 131072);
