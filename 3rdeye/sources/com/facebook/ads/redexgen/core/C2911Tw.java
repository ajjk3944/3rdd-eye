...
public class C2911Tw {
     ...
     */
    public static C2911Tw A04(File file) throws IOException {
        try {
            ...
            if (j4 != -360011992771067903L) {
                throw new IOException(String.format(Locale.US, A05(180, 27, 45), Long.valueOf(j4), -360011992771067903L));
            }
            ...
            if (j10 < 0 || j10 > 100000) {
                throw new IOException(String.format(Locale.US, A05(77, 40, 27), Long.valueOf(j10)));
            }
            if (randomAccessFile.length() < A02((int) j10)) {
                throw new IOException(String.format(Locale.US, A05(117, 63, 56), Long.valueOf(j10), Long.valueOf(file.length())));
            }
            long[] jArr = new long[(int) j10];
            for (int i = 0; i < jArr.length; i++) {
                jArr[i] = randomAccessFile.readLong();
            }
            return new C2911Tw(randomAccessFile, jArr, randomAccessFile.length());
    ...
    public C2911Tw(RandomAccessFile randomAccessFile, long[] jArr, long j4) throws IOException {
        ...
        for (int i10 = this.A00; i10 < jArr.length; i10++) {
            if (jArr[i10] != 0) {
                throw new IOException(String.format(Locale.US, A05(257, 66, 57), Integer.valueOf(i10), Integer.valueOf(this.A00), Long.valueOf(jArr[i10])));
            }
        }
        for (int i11 = 1; i11 < this.A00; i11++) {
            if (jArr[i11 - 1] > jArr[i11]) {
                throw new IOException(String.format(Locale.US, A05(207, 50, 76), Integer.valueOf(i11 - 1), Long.valueOf(jArr[i11 - 1]), Integer.valueOf(i11), Long.valueOf(jArr[i11])));
            }
        }
        if (this.A00 > 0 && jArr[this.A00 - 1] > j4) {
            throw new IOException(String.format(Locale.US, A05(323, 50, 117), Integer.valueOf(this.A00 - 1), Long.valueOf(jArr[this.A00 - 1]), Long.valueOf(j4)));
        }
    }
    ...
    public static C2911Tw A03(File file) throws IOException {
        RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(416, 3, 33));
        try {
            randomAccessFile.writeLong(-360011992771067903L);
            randomAccessFile.writeLong(C2912Tx.A03);
            randomAccessFile.write(new byte[C2912Tx.A03 * 8]);
            randomAccessFile.getFD().sync();
            return new C2911Tw(randomAccessFile, new long[C2912Tx.A03], randomAccessFile.length());
        } catch (IOException unused) {
